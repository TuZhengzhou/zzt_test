...
- a. 您好，您先需要启动FD的信号驱动功能
- b. 好的，已使用fcntl设置
- a. 接下来需要设置接受FD信号的进程
- b. 好的，已设置
- a. 您还需要设置和绑定信号动作结构体，其中信号动作结构体中需要指定回调函数、信号屏蔽设置、扩展行为启用设置
- a. 再提醒您一下，回调函数中您需要自行读取数据（从内核到用户缓冲区阶段）
- b. 好的
...
异步IO控制模块需要指定异步IO FD，异步IO完成通知方式 sigev_notify，异步IO完成后需要执行的操作 handler。

- a. 你好！你需要哪个FD执行异步IO？
- b. 指定FD
- a. 好的，请您提供异步IO所需的缓冲区
- b. 指定缓冲区信息
- a. 异步IO完成后怎么通知您，接下来要做什么？
- b. 指定通知方式和回调函数
- a. 如果您需要链式调用，只需要把控制块地址写入信号事件值联合体就行 `cb.aio_sigevent.sigev_value`
- b. 感谢提醒，设置好了
- b. 好的，您可以通过控制块提交异步请求了！
