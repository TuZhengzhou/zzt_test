...
void server_thread_review1() {
  ...
  /*
    设置异步读写FD
  */
  static struct aiocb ac;
  memset(&ac, 0, sizeof(aiocb));
  ac.aio_fildes = g_conn_fd;
  ...
  */
  ...
  /*
    设置异步读写通知方式和回调函数
  */
  ac.aio_sigevent.sigev_notify = SIGEV_THREAD;
  ac.aio_sigevent.sigev_notify_function = aio_completion_handler;

  /*
    传递链式调用所需的 cb
  */
  ac.aio_sigevent.sigev_value.sival_ptr = (void*)&ac;
